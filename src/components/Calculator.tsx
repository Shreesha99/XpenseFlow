...
export default function Calculator({ stats, compact }: CalculatorProps) {
  ...
  const actualBalance =
    (summary.digital_credits || 0) +
    (summary.in_hand_credits || 0) -
    ...
    return (
      <div className="space-y-6" id="calculator-compact">
        ...
        <div className="pt-4 border-t border-border flex justify-between items-center">
          ...
          <span className="text-xl font-bold font-mono text-emerald-500 tracking-tighter">
            ₹{(projectedBalance || 0).toLocaleString()}
          </span>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage your recurring financial commitments.
          </p>
        </div>
        <div className="flex gap-4">
          <div className="px-6 py-3 bg-card border border-border rounded-2xl shadow-sm">
            ...
            <p className="text-2xl font-bold font-mono text-emerald-500 tracking-tighter">
              ₹{(projectedBalance || 0).toLocaleString()}
            </p>
          </div>
        </div>
          ...
          <div className="bg-card border border-border rounded-3xl p-8 shadow-sm space-y-8">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Monthly recurring expenditure breakdown.
              </p>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                ...
                <span>
                  {Math.round((projectedBalance / (actualBalance || 1)) * 100)}%
                </span>
              </div>
              <div className="w-full bg-muted h-1.5 rounded-full overflow-hidden">
                <motion.div
                  ...
                  animate={{
                    width: `${Math.min(
                      Math.max(
                        (projectedBalance / (actualBalance || 1)) * 100,
                        0
                      ),
                      100
                    )}%`,
