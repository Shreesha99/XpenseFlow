      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="bg-[#151619] border border-[#2A2C32] rounded-3xl shadow-[0_40px_80px_rgba(0,0,0,0.5)] w-72 md:w-80 overflow-hidden mb-4 md:mb-6"
          >
            {/* Hardware Header */}
            <div className="px-6 py-4 bg-[#1A1C20] border-b border-[#2A2C32] flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)] animate-pulse" />
                <span className="text-[10px] font-mono font-bold text-[#8E9299] uppercase tracking-[0.2em]">
                  System.Calc_v2.1
                </span>
              </div>
              <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <CalcButton
                  label="%"
                  onClick={() => setDisplay((d) => String(parseFloat(d) / 100))}
                  variant="secondary"
                />
                <CalcButton
                  label="÷"
              ...
              <div className="flex items-center justify-between pt-2">
                <div className="flex gap-1">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className="w-1 h-1 rounded-full bg-[#2A2C32]"
                    />
                  ))}
                </div>
                <span className="text-[8px] font-mono text-[#4A4D54] uppercase tracking-widest">
                  Precision_Mode_Active
                </span>
              </div>
            </div>
          </motion.div>
...
function CalcButton({
  ...
  label: string;
  onClick: () => void;
  variant?: "default" | "operator" | "primary" | "secondary" | "danger";
  className?: string;
}) {
  const variants = {
    default:
      "bg-[#1A1C20] hover:bg-[#2A2C32] text-[#E0E0E0] border border-[#2A2C32]",
    operator:
      "bg-emerald-500/5 hover:bg-emerald-500/10 text-emerald-500 border border-emerald-500/20",
    primary:
      "bg-emerald-600 hover:bg-emerald-500 text-white shadow-[0_0_20px_rgba(16,185,129,0.2)]",
    secondary:
      "bg-[#1A1C20] hover:bg-[#2A2C32] text-[#8E9299] border border-[#2A2C32]",
    danger:
      "bg-rose-500/5 hover:bg-rose-500/10 text-rose-500 border border-rose-500/20",
  };
